...
summary(factor(merged_df_wo_duplicates$year))
merged_df_wo_duplicates$YearCycle <- "1999-2000"
...
Biomarker_NHANES_19992000<-merged_df_wo_duplicates[, c("seqn","lbdsalsi","lbdscrsi","lbdsglsi",
                                                       ...
                                                       "lbxsapsi","lbxwbcsi","ridageyr","YearCycle")]
...
merged_df_wo_duplicates <- merged_df %>% 
  distinct(seqn, .keep_all = TRUE)
merged_df_wo_duplicates$YearCycle <- "2001-2002"
...
Biomarker_NHANES_20012002<-merged_df_wo_duplicates[, c("seqn","lbdsalsi","lbdscrsi","lbdsglsi",
                                                       "lbxcrp","lbxlypct","lbxmcvsi","lbxrdw",
                                                       "lbdsapsi","lbxwbcsi","ridageyr","YearCycle")]
# Removing observations with missing values for each 
# lab value one by one to only select those observations
# in the end with no missing lab values
...
merged_df_wo_duplicates$YearCycle <- "2003-2004"
...
# of data) merged_df_wo_duplicates <- merged_df %>% 
merged_df_wo_duplicates <- merged_df %>% distinct(seqn, .keep_all = TRUE)
merged_df_wo_duplicates$YearCycle <- "2005-2006"
...
# of data) merged_df_wo_duplicates <- merged_df %>% 
merged_df_wo_duplicates <- merged_df %>% distinct(seqn, .keep_all = TRUE)
merged_df_wo_duplicates$YearCycle <- "2007-2008"
...
# of data) merged_df_wo_duplicates <- merged_df %>% 
merged_df_wo_duplicates <- merged_df %>% distinct(seqn, .keep_all = TRUE)
merged_df_wo_duplicates$YearCycle <- "2009-2010"
...
# of data) merged_df_wo_duplicates <- merged_df %>% 
merged_df_wo_duplicates <- merged_df %>% distinct(seqn, .keep_all = TRUE)
merged_df_wo_duplicates$YearCycle <- "2015-2016"
...
# of data) merged_df_wo_duplicates <- merged_df %>% 
merged_df_wo_duplicates <- merged_df %>% distinct(seqn, .keep_all = TRUE)
merged_df_wo_duplicates$YearCycle <- "2017-2018"
...
NHANES_Variables_19992020$CurrentFormerORNeverSmoker<-as.factor(ifelse(NHANES_Variables_19992020$smq020=="1","Current or Former Smoker",
                                                         ifelse(NHANES_Variables_19992020$smq020=="2","Never Smoker",NA)))
summary(NHANES_Variables_19992020$smq040=="3" & NHANES_Variables_19992020$CurrentFormerORNeverSmoker=="Current or Former Smoker")

NHANES_Variables_19992020$CurrentSmokingFrequency<-as.factor(ifelse(NHANES_Variables_19992020$smq040=="3","Not at all",
                                                      ifelse(NHANES_Variables_19992020$smq040=="1"|
                                                               NHANES_Variables_19992020$smq040=="2","Smoking",NA)))

NHANES_Variables_19992020$SmokingStatus<-as.factor(ifelse(NHANES_Variables_19992020$CurrentFormerORNeverSmoker=="Current or Former Smoker" &
                                                            NHANES_Variables_19992020$CurrentSmokingFrequency=="Smoking","Current Smoker",
                                            ifelse(NHANES_Variables_19992020$CurrentFormerORNeverSmoker=="Current or Former Smoker" &
                                                     NHANES_Variables_19992020$CurrentSmokingFrequency=="Not at all","Former Smoker","Never Smoker")))
summary(NHANES_Variables_19992020$SmokingStatus)

# Alcohol
NHANES_Variables_19992020$Alcohol_HeavyDrinking<-as.factor(ifelse(is.na(NHANES_Variables_19992020$alq150),paste(NHANES_Variables_19992020$alq151),paste(NHANES_Variables_19992020$alq150)))
summary(factor(NHANES_Variables_19992020$Alcohol_HeavyDrinking))
NHANES_Variables_19992020$Alcohol_AnyDrinking<-as.factor(ifelse(is.na(NHANES_Variables_19992020$alq100),paste(NHANES_Variables_19992020$alq101),paste(NHANES_Variables_19992020$alq100)))
...
NHANES_Variables_19992020$Told_Had_Diabetes<-as.factor(ifelse(NHANES_Variables_19992020$diq010=="1","Yes",
                                                                     ifelse(NHANES_Variables_19992020$diq010=="2","No",NA)))
NHANES_Variables_19992020$Diabetes<-as.factor(ifelse(NHANES_Variables_19992020$Told_Had_Diabetes=="Yes"|
                                                       NHANES_Variables_19992020$Diabetes_Based_on_fastingGlucose=="Yes","Yes","No"))
...
Final_NHANES_19992020<-merge(NHANES_Variables_19992020,
                             ...
                             by="seqn")
Final_NHANES_19992020$AgeGroups<-as.factor(ifelse(Final_NHANES_19992020$ridageyr<40,"19-39",
                                                                       ifelse(Final_NHANES_19992020$ridageyr>39&
                                                                                Final_NHANES_19992020$ridageyr<60,"40-59",
                                                                              ifelse(Final_NHANES_19992020$ridageyr>59,"60+",NA))))
################################################################################
# General Information #
################################################################################
...
Final_NHANES_19992020_nonmissing$Phenotypic_Age<-Final_NHANES_19992020_nonmissing$PhenoAge
nhanesDesign <- svydesign(id      = ~sdmvpsu,
                          strata  = ~sdmvstra,
                          weights = ~wtint2yr,
                          nest    = TRUE,
                          data    = Final_NHANES_19992020_nonmissing)
total_patients <- sum(nhanesDesign$weights)
...
phenotypicage_agegroups_summary <-data.frame(phenotypicage_agegroups)
...
# View the results
print(phenotypicage_agegroups_summarysd)
...
# View the results
print(Difference_Age_Phenotypic_Age_agegroups_summarysd)
...
phenotypicage_agegroups_summarysd2
# Calculate the mean and standard deviation of Phenotypic_Age by age_deciles
...
# View the results
print(phenotypicage_sex_summarysd)
...
# View the results
print(phenotypicage_BMI_Groups_summarysd)
...
#                No 1322298535      91.39%
#               Yes  124521799       8.61%
phenotypicage_Diabetes <- svyby(~Phenotypic_Age, ~factor(Diabetes), design = nhanesDesign, svymean)
phenotypicage_Diabetes_sd <- svyby(~Phenotypic_Age, ~factor(Diabetes), design = nhanesDesign, svyvar)
phenotypicage_Diabetes_range <- svyby(~Phenotypic_Age, ~Diabetes, design = nhanesDesign, FUN = svyquantile, quantiles = c(0, 1))
...
phenotypicage_Hypertension <- svyby(~Phenotypic_Age, ~factor(Hypertension), design = nhanesDesign, svymean)
phenotypicage_Hypertension_sd <- svyby(~Phenotypic_Age, ~factor(Hypertension), design = nhanesDesign, svyvar)
phenotypicage_Hypertension_range <- svyby(~Phenotypic_Age, ~Hypertension, design = nhanesDesign, FUN = svyquantile, quantiles = c(0, 1))
...
# Calculate mean, sd, and 95% CI for lbdsalsi variable
# Extract the mean, sd, and CI from the summary stats
mean_lbdsalsi <- svymean(~lbdsalsi, nhanesDesign)
sd_lbdsalsi <- svysd(~lbdsalsi, design = nhanesDesign)
lbdsalsi_percentiles <- svyquantile(~lbdsalsi, nhanesDesign, c(0.025, 0.975))

# Calculate mean and standard deviation of lbxhscrp
sd_lbdsalsi <- svysd(~lbdsalsi, design = nhanesDesign)
library(ggplot2)
...
percentiles <- quantile(lbdsalsi, probs = c(0.025, 0.975))

hist <- svyhist(~lbdsalsi, design = nhanesDesign, main = "Histogram of lbdsalsi variable")

# Add text for 2.5th and 97.5th percentiles
text(hist$mids[2], max(hist$counts), "2.5th percentile")
text(hist$mids[length(hist$mids) - 1], max(hist$counts), "97.5th percentile")

# Add a line for the 2.5th percentile
abline(v = quantile(nhanesDesign$lbdsalsi, probs = 0.025), lty = 2)

pdf("plot_histogram_nhanes_albumin.pdf")


library(ggplot2)

# Create a data frame with lbdsalsi variable
library(ggplot2)

df <- data.frame(lbdsalsi = nhanesDesign$lbdsalsi)

ggplot(df, aes(x = lbdsalsi)) +
  geom_histogram(binwidth = 1, color = "black", fill = "white") +
  scale_x_continuous(limits = c(0, 200), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0, 100000000), expand = c(0, 0)) +
  labs(x = "lbdsalsi variable", y = "Count", 
       title = "Histogram of lbdsalsi variable") +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5), 
        axis.title = element_text(size = 12),
        axis.text = element_text(size = 10)) +
  # Define the count variable using stat_bin()
  stat_bin(aes(y = ..count..)) +
  # Add text for 2.5th and 97.5th percentiles
  geom_text(aes(x = quantile(lbdsalsi, probs = 0.025), 
                y = max(..count..), 
                label = "2.5th percentile"), 
            vjust = -1, hjust = -0.2) +
  geom_text(aes(x = quantile(lbdsalsi, probs = 0.975), 
                y = max(..count..), 
                label = "97.5th percentile"), 
            vjust = -1, hjust = 1.2) +
  # Add a line for the 2.5th percentile
  geom_vline(aes(xintercept = quantile(lbdsalsi, probs = 0.025)), 
             linetype = "dashed") +
  # Add a line for the 97.5th percentile
  geom_vline(aes(xintercept = quantile(lbdsalsi, probs = 0.975)), 
             linetype = "dashed")
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
